...
#include <mpc_pkg/OptiDataQDMCWind.h>
...
void inidataReceived(const mpc_pkg::OptiDataQDMCWind& msg) {
  ...
  in_params.block(1,0, 1,6) 	 = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>
	...
	Eigen::VectorXd object_constraint(Hp);
	for(int i = 1; i < Hp + 1; ++i){
	  if (in_params(4, i) <= -0.6){
	    object_constraint(i - 1) = -20.0; //-0.28;
	  }else{
	    object_constraint(i - 1) = -20.0;
	  }
	}
	in_params.block(8, 0, 1, Hp) = object_constraint.transpose();
	


	in_params.block(9, 0, 1, Hp) = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>
                                 (((vector<double>) msg.disturbance_profile_x).data(),  Hp).transpose();

  in_params.block(10, 0, 1, Hp) = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>
                                 (((vector<double>) msg.disturbance_profile_y).data(),  Hp).transpose();

  in_params.block(11, 0, 1, Hp) = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>
                                 (((vector<double>) msg.disturbance_profile_z).data(),  Hp).transpose();
...
int main(int argc, char **argv) {
  ...
  if(!rosnh.getParam("/qdmc_opti_node/noise_var", noise_var)) {
    ROS_ERROR("Need to define the noise_var (noise variance on the states) parameter.");
  }
  if(!rosnh.getParam("/qdmc_opti_node/traj_length", traj_length)) {
    ROS_ERROR("Need to define the traj_length (length of the trajectory) parameter.");
  }
  ...
  Eigen::MatrixXd D_tilde = getCSVcontent(datapath+"D_tilde.csv");
  ...
  DM D_tilde_DM = DM::zeros(6 * Hp, 3 * Hp);
  memcpy(D_tilde_DM.ptr(), D_tilde.data(), sizeof(double) * 6 * Hp * 3 * Hp);
	
	// Initial parameters of the optimization problem
	SX P = SX::sym("P", 1 + 1 + 6 + 1 + 3, 6 * Hp);
  SX f_tilde = P(0, Slice()).T();
  SX u0 = P(1,Slice(0,6)).T();
  SX Rp_mat = P(Slice(2,2+6), Slice(1, Hp + 1));
  SX Rp = Rp_mat(0, Slice()).T();
  SX disturbance_profile_mat = P(Slice(9, 9 +3), Slice(0, Hp));
  SX disturbance_profile = disturbance_profile_mat(0, Slice()).T();
  for(int i=1; i<6; ++i){
      Rp = vertcat(Rp, Rp_mat(i, Slice()).T());
  }
  
  for(int i=1; i<3; ++i){
      disturbance_profile = vertcat(disturbance_profile, disturbance_profile_mat(i, Slice()).T());
  }
  ...
  // Calculate the noise variance to be used in the chance constraints.
  Eigen::MatrixXd identity_matrix_trajectory = Eigen::MatrixXd::Identity(traj_length, traj_length);
  Eigen::MatrixXd sigma_epsilon_i = noise_var * identity_matrix_trajectory;
  Eigen::MatrixXd padded_identity = Eigen::MatrixXd::Identity(Hp, traj_length);
  Eigen::MatrixXd Xi = padded_identity;
  Eigen::MatrixXd zero_padding = Eigen::MatrixXd::Zero(Hp, 1);
  Eigen::MatrixXd Nu(Hp, traj_length);
  Nu << zero_padding, padded_identity.block(0, 1, Hp, traj_length - 1);
  Eigen::MatrixXd sigma_nu_i = (Nu - Xi) * sigma_epsilon_i * (Nu - Xi).transpose();
  Eigen::MatrixXd sigma_nu_tilde(3 * sigma_nu_i.rows(), 3 * sigma_nu_i.cols()); //the disturbance acts on the three Cartesian axes
  sigma_nu_tilde.block(0, 0, sigma_nu_i.rows(), sigma_nu_i.cols()) = sigma_nu_i;
  sigma_nu_tilde.block(sigma_nu_i.rows(), sigma_nu_i.cols(), sigma_nu_i.rows(), sigma_nu_i.cols()) = sigma_nu_i;
  sigma_nu_tilde.block(2*sigma_nu_i.rows(), 2*sigma_nu_i.cols(), sigma_nu_i.rows(), sigma_nu_i.cols()) = sigma_nu_i;
  Eigen::MatrixXd sigma_y_tilde = D_tilde * sigma_nu_tilde * D_tilde.transpose();
  DM sigma_y_tilde_DM = DM::zeros(sigma_y_tilde.rows(), sigma_y_tilde.cols());
  memcpy(sigma_y_tilde_DM.ptr(), sigma_y_tilde.data(), sizeof(double) * sigma_y_tilde.rows() * sigma_y_tilde.cols());
  ...
  SX y = f_tilde + SX::mtimes(S_tilde_DM, w) + SX::mtimes(D_tilde_DM, disturbance_profile);
  SX error = y - Rp;
  ...
  double alpha = 0.1;
  //TODO remove hardcoded constant for inverse normal cdf (use e.g. boost math library) 
  double inverse_cdf = 4.456436336204150;
  //New constraint: upper bound on z axis equal to 0.2
  SX constraint = P(8, Slice(0, Hp)).T();
  for (int k=0; k<2*Hp; k++) {            
    // Constraint: Constant distance between upper corners
    g = vertcat(g, y(4 * Hp + k) - constraint(k%Hp));
    lbg.insert(lbg.end(), 1, inverse_cdf * sqrt(sigma_y_tilde(4*Hp + k, 4*Hp + k)));
    ROS_WARN_STREAM(inverse_cdf * sqrt(sigma_y_tilde(4*Hp + k, 4*Hp + k)));
    ubg.insert(ubg.end(), 1, inf);
  }
  ...
  Eigen::VectorXd u6_rot;
	...
	in_params.resize(1 + 1 + 6 + 1 + 3, 6*Hp);
	// Wait for initial optidata
	ROS_INFO_STREAM("Initialized Optimizer");
	boost::shared_ptr<mpc_pkg::OptiDataQDMCWind const> OptiData0;
	OptiData0 = ros::topic::waitForMessage<mpc_pkg::OptiDataQDMCWind>("/mpc_controller/opti_inidata");
