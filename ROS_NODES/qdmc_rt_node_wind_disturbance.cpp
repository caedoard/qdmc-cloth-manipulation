...
Eigen::VectorXd d_bef = Eigen::VectorXd::Zero(3, 1);
Eigen::MatrixXd current_disturbance;
Eigen::MatrixXd B_delta;
...
void somstateReceived(const mpc_vision::SOMstate& msg) {
  ...
  Eigen::VectorXd VSOMstate_rot(6*nSOM*nSOM);
	...
	for (int Vti=-update_steps; Vti<0; Vti++) {
		...
		// Simulate a step
		VSOMstate_rot = A_SOM*VSOMstate_rot + B_SOM*urot_Vti + Ts*f_SOM + B_delta * (current_disturbance); 
...
int main(int argc, char **argv) {
  ...
  // Define Publishers
  ros::Publisher pub_inidata = rosnh.advertise<mpc_pkg::OptiDataQDMCWind>
  ...
  Eigen::MatrixXd disturbance_profile = getCSVcontent(datapath+"disturbance_profile.csv").transpose();
  ...
  Eigen::MatrixXd D_tilde_ss_full = getCSVcontent(datapath+"D_tilde_ss.csv");
  Eigen::MatrixXd B_delta = getCSVcontent(datapath+"B_delta.csv");
  ...
  Eigen::SparseMatrix<double> D_tilde_ss = D_tilde_ss_full.sparseView();
  Eigen::VectorXd d_bef = Eigen::VectorXd::Zero(3, 1);
	...
	while(rosnh.ok()) {
		...
		// Check subscriptions
		ros::spinOnce();
		// Slice disturbance and compute deltas, and rotate
    Eigen::MatrixXd sliced_disturbance;
    if(tk >= disturbance_profile.cols()-Hp) {
	    sliced_disturbance = (Eigen::VectorXd::Ones(Hp)*disturbance_profile.rightCols(1).transpose()).transpose();
	  }else{
	    sliced_disturbance = disturbance_profile.block(0,tk, 3,Hp);
	  }

		for(int i = 1; i < Hp; ++i){
		  sliced_disturbance.col(i) = sliced_disturbance.col(i) - sliced_disturbance.col(i - 1);
		  sliced_disturbance.col(i) = Rcloth.inverse() * sliced_disturbance.col(i);
		}	

		sliced_disturbance.col(0) = sliced_disturbance.col(0) - d_bef;
		sliced_disturbance.col(0) = Rcloth.inverse() * sliced_disturbance.col(0);
		current_disturbance = sliced_disturbance.col(0) + d_bef; // dbef is in cloth base
    ...
    f_tilde = Psi * f_i_curr + d_model_mismatch;                
		...
		mpc_pkg::OptiDataQDMCWind optiData_pub;
		...
		Eigen::VectorXd dist_x = sliced_disturbance.row(0);	
		Eigen::VectorXd dist_y = sliced_disturbance.row(1);						
		Eigen::VectorXd dist_z = sliced_disturbance.row(2);											
		optiData_pub.disturbance_profile_x = vector<double>(dist_x.data(),
							 dist_x.size()+dist_x.data());

		optiData_pub.disturbance_profile_y = vector<double>(dist_y.data(),
							 dist_y.size()+dist_y.data());

		optiData_pub.disturbance_profile_z = vector<double>(dist_z.data(),
							 dist_z.size()+dist_z.data());
		...
		// Simulate a step
	  state_SOM_rot = A_SOM*state_SOM_rot + B_SOM*u_rot + Ts*f_SOM + B_delta * current_disturbance;
    ...
    Eigen::VectorXd sliced_disturbance_padded = Eigen::VectorXd::Zero(3 * Hp);
		for(int i = 0; i < 3; ++i){
        sliced_disturbance_padded(i * Hp) = sliced_disturbance(i, 0);
    }
		f_i_curr = Gamma * f_i_curr + S_tilde_ss * u_rot_padded + D_tilde_ss * sliced_disturbance_padded;
		d_bef = current_disturbance;
