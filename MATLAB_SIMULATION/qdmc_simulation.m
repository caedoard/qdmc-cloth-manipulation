...
for traj_index = 10
    ...
    %             set(gca, 'TickLabelInterpreter', 'latex');
            ...
            P = SX.sym('P', H * n_y, 4); % Parameters. First col is f_tilde_k, second 
                                          % col is reference signal, third col is
                                          % predicted disturbance, fourth col
                                          % is previous delta (deltau) (upper
                                          % corner accel)
            ...
            error = P(:, 1) + S_tilde * U - P(:, 2) + D_tilde * P(1:n_d * H, 3); % F_tilde + S_tilde * delta_u - reference + D_tilde * delta
    ...
    % hold off
                ...
                args.p = [f_tilde_i, reference, predicted_disturbance, padded_ubef];
                t0 = tic;
...
%     writematrix(errors, "qdmc_error_sim_traj" + traj_index +".csv");
%     writematrix(runtimes, "qdmc_time_sim_traj" + traj_index +".csv");
%     writematrix(tottimes, "qdmc_tot_time_sim_traj" + traj_index +".csv");
